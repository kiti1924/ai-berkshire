...
調査開始時に次の構成をユーザーへ示し、各役割を明確にする。対象市場が **日本市場 (`JP`)** の場合は、日本固有の資本配分・ガバナンス（村上世彰型）および組織成長力（藤野英人型）の視点を自動統合する。

| 役割 | 責任 | 主な分析枠組み（グローバル / 日本株JPモード） |
|---|---|---|
| **team-lead**（自分） | 調整、論点統合、最終判断、報告作成 | 観点の統合および東証資本効率・ガバナンス評価 |
| **business-analyst** | 事業モデルと経済的なモート | 段永平の事業観 ＋ **藤野英人型（組織能力・後継者・成長の兆候）** |
| **financial-analyst** | 財務諸表とバリュエーション | バフェットの価値評価 ＋ **村上世彰型（余剰現金・政策保有株・親子上場・資本効率改善）** |
| **industry-researcher** | 業界構造と競争環境 | マンガーの多面的・逆向き思考（組織的現状維持・停滞リスク） |
| **risk-assessor** | リスク、経営陣、長期予測可能性 | 李録の長期・文明進化（人口減少耐性・省人化/FA・グローバル需要） |

※ユーザーから「詳細調査」または「6Agent構成」の指定があった場合は、`capital-allocation` (村上型) および `management-growth` (藤野型) を独立した専任Agentとして起動する6Agent構成へ拡張してよい。
...
- `subject`: `{会社名}の事業モデル、経済的なモート、顧客価値を分析する`
- `description`に含める内容：
  1. 事業の本質と売上構成。
  ...
  7. **（日本株JPモード時）** 藤野英人型の視点：組織がこれから成長する兆候、現場への権限委譲、技術継承、後継者育成、不採算事業を止める能力を評価する。
  8. 最新の決算資料、法定開示、業界資料を検索する。
...
- `description`に含める内容：
  ...
  6. **（日本株JPモード時）** 村上世彰型の視点：余剰現金/時価総額、政策保有株式/純資産、親子上場・支配株主リスク、東証資本コスト・株価意識経営計画、調整後EV（時価総額+負債-余剰現金-政策保有株）を算出評価する。
  7. 内在価値と現在価格から安全余裕を評価する。
  8. 次のコマンドで重要計算を検証し、暗算だけで確定しない。
...
- `description`に含める内容：
  ...
  8. **（日本株JPモード時）** 李録型の視点：日本国内の人口減少を補う自動化/FA/省人化需要、グローバル市場での競争力、円安影響の実質評価を行う。
